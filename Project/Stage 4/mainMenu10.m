...
while loopAgain == true
    ...
    % taking user output
        if option == 1 % Calculate molar mass (M)
            elements = input("\nInput the elements of the molecules in the form of: \n" + ...
            "Element1 Element2 Element1 in proper casing and \n" + ...
            "Enter any element more than once if required:\n", "s");
            if elements == 00 % condition to loop back to the menu
                loopAgain = true; % to loop back to the menu
            else
                element = split(elements, " ");
                outputMolarMass = molarMass(element); % using the molarMass function
        ...
        elseif option == 2 % Convert degrees Celsius to Kelvin
            ...
            if tempInCelsius == 00 % condition to loop back to the menu
                loopAgain = true; % to loop back to the menu
            else
        ...
        elseif option == 3 % Calculate an unknown value in Ideal Gas Law
            valuesGivenStringInput = input("\nInput the values in the form of: \n" +...
                "(where 8.314 J mol^-1 K^-1 and 0.08206 L atm K^-1 mol^-1) \n"+ ...
                "P,V,n,R, T but use a zero '0' for the unknown value:\n", "s"); % (from below)
            % own code from here
            if valuesGivenStringInput == "00" % condition to loop back to the menu
                loopAgain = true; % to loop back to the menu
            else
                ...
                outputUnknown = idealGasLaw(valuesGiven(1),valuesGiven(2),valuesGiven(3),valuesGiven(4),valuesGiven(5));
                fprintf("\nunknown = %0.8f \n",outputUnknown);
                loopAgain = true; % automatically loop so the user does not have to keep re-running program
            end
        ...
        property = input("\nEnter property to find possibilities of spontaneity using:\n" + ...
            "'+deltaH' or '-deltaH' or\n" + ...
            "'high temperatures' or 'all temperatures' or 'low temperatures' or\n" + ...
            "'+deltaS' or '-deltaS':\n","s");

        if property == "00" % condition to loop back to the menu
            loopAgain = true; % to loop back to the menu
        else
            ...
            indexes = []; % inititalise empty vector as reference for which row to print of spontaneous
            indexeN = []; % inititalise empty vector as reference for which row to print of non-spontaneous
            ...
            fprintf("The reaction is spontaneous when:\n") 
            disp(possibilities); % disp can print the possibilities not sure how to optimise printing yet
            ...
            for i = 1:rowNs
                for j = 1:colNs
                     if strcmp(nonSpontaneous(i,j),property) == 1 % comparing the property with what is in non-spontaneous
                         indexeN = [indexeN, i]; % reference for which row to print
                     end
                end
                possibilities = nonSpontaneous(indexeN,:); % stored in here to print
            end
            
            fprintf("The reaction is non-spontaneous when:\n")
            disp(possibilities); % disp can print the possibilities not sure how to optimise printing yet
        end
        ...
        imshow("quadraticFormula.png"); % display as an image
        valuesStringInput = input("\nInput the values in the form of: \n" + ...
            "a,b,c,minValue x can be, maxValue x can be:\n", "s"); % (ChatGPT suggested turn into string)
        % own code from here
        if valuesStringInput == "00" % condition to loop back to the menu
            loopAgain = true; % to loop back to the menu
        else
            ...
            fprintf("\nx = %0.8f \n",outputValOfX); % printing with 8 decimal places
            loopAgain = true; % automatically loop so the user does not have to keep re-running program
        end
